...
Another function that is useful for evaluating the gene calling results is ```iss_view_spot```, which plots the location around a spot in each round and colour channel. It can be used in collaboration with any of the gene calling methods. It is run through: ```iss_view_spot(o, 234321, ImSz, SpotLocation, ScoreMethod, IncludeGT, Filter, Norm, SpotNo)```. Here, ```Norm``` is either ```true``` or ```false``` to plot the channel/round normalised or raw colors. ```Filter``` is either ```true``` or ```false``` to plot the colors after or before the ```extract_and_filter``` step. All the other arguments are as described in previous functions.
