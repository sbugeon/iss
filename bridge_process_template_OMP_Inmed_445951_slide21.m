...
for iSlice = 1:length(SliceList)
    ...
    o.FileBase = cell(1,1);
    o.FileBase{1} = strcat(SliceNb,'03');
    o.FileBase{2} = strcat(SliceNb,'02');
    o.FileBase{3} = strcat(SliceNb,'04');
    o.FileBase{4} = strcat(SliceNb,'05');
    o.FileBase{5} = strcat(SliceNb,'06');
    o.FileBase{6} = strcat(SliceNb,'07');
    o.FileBase{7} = strcat(SliceNb,'08');
    o.FileBase{8} = strcat(SliceNb,'11');
    ...
    for j = 15%1:length(o.TileConnectedID)
        ...
        o.FindSpotsSearch.X = -500:o.FindSpotsStep(2):500;
...
o.iss_change_plot('OMP',[], {'Grik1','Chrna6','Mia','Ntng1'});
...
%% find tdTomato cells
GUI_findtdTomato('D:\ISS\445951\Slide21\Round-\output')
...
for i=1:length(D)
    AllSessionROI2.(D{i}) = AllSessionROI.(D{i})(~strcmp( AllSessionROI.cluster,'Zero') & ~strcmp( AllSessionROI.cluster,'Not_neuron'),:);%& MeanGC>-Inf
end
