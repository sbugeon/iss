function [shift, cc, fa1, fa2] = ImRegFFt3D_Register(o,Im1, Im2, CorrThresh, MinSize,Direction)
...
if strcmp(Direction,'South')
    if ShiftTry(1)<-sz(1)*(1-o.MaxOverlapFract) && abs(ShiftTry(2))<o.MaxRegShift
        ...
        while i<=floor(o.ToTest*size(sorted,1))
            ...
            ShiftTry = mod([dy0, dx0, dz0] +sz, sz*2) - sz - 1;
            if ShiftTry(1)<=-sz(1)*(1-o.MaxOverlapFract) && ShiftTry(1)>-sz(1)+1 && abs(ShiftTry(2))<=o.MaxRegShift                
                cc = sorted(i);
                if cc>CorrThresh(1)
                    shift = ShiftTry;
                i = size(sorted,1)+1;
...
elseif strcmp(Direction,'East')
    if ShiftTry(2)<=-sz(1)*(1-o.MaxOverlapFract) && abs(ShiftTry(1))<=o.MaxRegShift
        ...
        while i<=floor(o.ToTest*size(sorted,1))
            [dy0, dx0,dz0] = ind2sub(size(Conv), order(i));
            ShiftTry = mod([dy0, dx0, dz0] +sz, sz*2) - sz - 1;
            if ShiftTry(2)<-sz(1)*(1-o.MaxOverlapFract) && ShiftTry(2)>-sz(1)+1 && abs(ShiftTry(1))<o.MaxRegShift
                cc = sorted(i);
                if cc > CorrThresh(1)
                    shift = ShiftTry;
                i = size(sorted,1)+1;
